...
int build_grating(char *filename, double duration, double angle, double sf, double tf, double contrast, int background, int width, int height, int waveform, double percent_sigma, double percent_diameter, double percent_center_left, double percent_center_top, double percent_padding, int colormode)
{
...
double *display_grating(void *frame_data, fb_config *fb0, int trig_pin, int colormode)
{
	...
	// Initial trigger to start (2022/07/18 wi)
	...
	// main loop show gratings (2022/07/18 wi)
	for (t = 0; t < n_frames; t++)
	{
		frame_end = frame_start;
		frame_start = get_current_time(&clock_status);
		...
		// stop if trigger pin is low (2022/07/18 wi)
		if (trig_pin > 0)
		{
			pinMode(trig_pin, INPUT);
			while (digitalRead(trig_pin) == 0)
